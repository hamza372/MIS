defmodule Sarkar.Server.Analytics do
	...
	def init(%{bindings: %{type: "platform-events.csv"}} = req, state) do
		...
		{:ok, data} = case Postgrex.query(Sarkar.School.DB,
		"SELECT id, to_timestamp((value->>'time')::bigint/1000)::date as date, value->>'event' as event, value->'meta'->>'call_status' as call_status, value->'meta'->>'duration' as duration_seconds
		FROM platform_writes
	...
	def init(%{bindings: %{type: "platform-call-surveys.csv"}} = req, state) do
		...
		csv = [ [ "supplier_id", "date", "event", "school_id", "customer_interest", "reason_rejected", "other_reason_rejected", "customer_likelihood", "follow_up_meeting", "other_notes" ] | data]
		|> CSV.encode
		|> Enum.join()
