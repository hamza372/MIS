defmodule Sarkar.Auth do
	...
	def verify({school_id, token}) do
		case Postgrex.query(Sarkar.School.DB,
		"SELECT * FROM tokens WHERE id=$1 AND token=$2",
		[school_id, token]) do
	...
	def gen_token(school_id) do
		...
		case Postgrex.query(Sarkar.School.DB, "INSERT INTO tokens (id, token) values ($1, $2)", [school_id, hash(token, 12)]) do
			{:ok, res} -> {:ok, token}
			{:error, err} -> 
				IO.inspect err
