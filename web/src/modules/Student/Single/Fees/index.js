...
class StudentFees extends Component {
	...
	componentDidMount() {
		...
		for(let [id, fee] of Object.entries(this.student().fees)) {
			if(fee.period === "MONTHLY") {
				// check if this fee exists in "owed" column.
				const existing_monthly = Object.values(this.student().payments || {}).find(p => p.fee_id === id && moment(p.date).format("MM/YYYY") === curr);
				if(existing_monthly === undefined) { // there is no payment for this month owed yet
					// create it
					const amount = (fee.type === "FEE" ? 1 : -1) * fee.amount;
					this.props.addPayment(this.student(), id, amount, moment().startOf('month').unix() * 1000, "OWED", id);
				}
			}
		}
		...
		return <div className="student-fees">
				...
				</div>
			{
				...
				<div className="table row">
					...
					<div><b>{Object.values(this.student().payments || {}).reduce((agg, curr) => agg - (curr.type === "SUBMITTED" || curr.type === "FORGIVEN" ? 1 : -1) * curr.amount, 0)}</b></div>
				</div>
			</div>
